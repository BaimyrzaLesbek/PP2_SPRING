    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if previous_pos and drawing == "line":
                pygame.draw.line(SCREEN, right_moment_color, previous_pos, current_pos, 1)
                previous_pos = current_pos
            if previous_pos and drawing == "eraser":
                pygame.draw.line(SCREEN, (255,255,255), previous_pos, current_pos, 10)
                previous_pos = current_pos
        if event.type == pygame.MOUSEBUTTONUP:
            if drawing == "circle" and event.button == 1:
                pygame.draw.circle(SCREEN, right_moment_color, previous_pos, abs(current_pos[0]-previous_pos[0]), 1)
            elif drawing == "circle" and event.button == 3:
                pygame.draw.circle(SCREEN, right_moment_color, previous_pos, abs(current_pos[0]-previous_pos[0]))
                previous_pos = None
            elif previous_pos and drawing == "rectangle" and event.button == 1:
                pygame.draw.rect(SCREEN, right_moment_color,[previous_pos[0],previous_pos[1], abs(current_pos[0]-previous_pos[0]),abs(current_pos[1]-previous_pos[1])], 1)
                previous_pos = None
